...
namespace Infrastructure.Services.Pricing.Quotation
{
    public class QuoteService : IQuoteService
    {
        ...
        public async Task<QuoteResponse> CreateAsync(CreateQuoteRequest dto, string userId)
        {
            return await ExecuteInTransactionAsync(async () =>
            {
                ...
                var audit = new AuditLog
                {
                    ...
                    EntityId = quote.Id,
                    EntityName = nameof(Quote).ToUpper(),
                    Action = nameof(CreateAsync).ToUpper(),
                    IpAddress = await IpAddressHelper.GetRealPublicIpAsync(),
                    OldValues = null,
                    NewValues = JsonSerializer.Serialize(quote),
                    UserId = userId
                };
        ...
        public async Task<QuoteResponse> AcceptFromUserAsync(Guid id, string userId)
        {
            return await ExecuteInTransactionAsync(async () =>
            {
                ...
                var audit = new AuditLog
                {
                    ...
                    EntityId = quote.Id,
                    EntityName = nameof(Quote).ToUpper(),
                    Action = nameof(AcceptFromUserAsync).ToUpper(),
                    IpAddress = await IpAddressHelper.GetRealPublicIpAsync(),
                    OldValues = JsonSerializer.Serialize(oldQuote),
                    NewValues = JsonSerializer.Serialize(quote),
                    UserId = userId
                };
        ...
        public async Task<QuoteResponse> RejectFromUserAsync(Guid id, string userId, string reason)
        {
            return await ExecuteInTransactionAsync(async () =>
            {
                ...
                var audit = new AuditLog
                {
                    ...
                    EntityId = quote.Id,
                    EntityName = nameof(Quote).ToUpper(),
                    Action = nameof(RejectFromUserAsync).ToUpper(),
                    IpAddress = await IpAddressHelper.GetRealPublicIpAsync(),
                    OldValues = JsonSerializer.Serialize(oldQuote),
                    NewValues = JsonSerializer.Serialize(quote),
                    UserId = userId
        ...
        public async Task DeleteAsync(Guid id, bool isAdmin, string userId)
        {
            await ExecuteInTransactionAsync(async () =>
            {
                ...
                var audit = new AuditLog
                {
                    ...
                    IpAddress = await IpAddressHelper.GetRealPublicIpAsync(),
                    OldValues = JsonSerializer.Serialize(quote),
                    NewValues = "Deleted",
                    UserId = userId
                };
